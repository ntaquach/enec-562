...
```{r message=F, warning=F, fig.align='center', fig.height=7, fig.width=8}
...
# Train the  model
model_best <- glm(stat ~ WT + TL + HL + KL, data=train.data, family = "binomial"(link = logit))
...
test_pred_60 = get_logistic_pred(model_best, data = test.data, res = "default", 
                                 pos = 1, neg = 0, cut = 0.6)

test_tab_60 = table(predicted = test_pred_60, actual = test.data$stat)
...
library(pROC)
best_prob = predict(model_best, newdata = test.data, type = "response")
best_roc = roc(test.data$stat ~ best_prob, plot = TRUE, print.auc = TRUE)
```
