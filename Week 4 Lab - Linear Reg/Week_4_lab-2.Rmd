...
```{r}
tdat <-read.csv("Data/TreePlots.csv", header=T)
```
...
Now let’s do a linear regression on tree height and aboveground biomass. Our hypothesis is that plots with greater mean height of trees have greater biomass. (You don’t have to be a genius to figure that out, but let’s proceed anyway.) Below, run the model, then test the H0 : mH.m = 0, then plot the data.
