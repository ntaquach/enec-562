...
```{r warning=F, message=F, echo=F}
  ...
  ungroup() %>% data.frame() %>%
  kable("latex", booktabs = T, escape = F,
        col.names = linebreak(c("Location", "Mean\nMethane\nConc.",
                                "Median\nMethane\nConc.", "Min\nMethane\nConc.",
                                "Max\nMethane\nConc.", "Standard Deviation of\nMethane\nConc."
        ), align = "c"))  %>%
  column_spec(1:1, width = "1.5cm") %>%
...
## test for normality
pafrack %>% group_by(proximity) %>%
  shapiro_test(methane) ## Okay so p-value is significant => either log transform data or non-parametric
...
## verify this: 
shapiro_test(log(pafrack$methane)) # Yes still non significant. Gotta do non parametric now
...
near_far_upland <-  upland_sites %>%
  wilcox_test(methane ~ proximity, alternative = "two.sided") %>%
  add_significance()
near_far_upland
