...
```{r message=F, warning=F, results=F}
dlab_tukey <- dlabs %>% tukey_hsd(results ~ antibiotic + lab)
dlab_tukey
```
Tukey HSD testn showed that the average levels of antibodies were different among:

a. Antibiotic 1 and 3 (p = `r dlab_tukey$p.adj[dlab_tukey$term == "antibiotic" & dlab_tukey$group1 == 1 & dlab_tukey$group2 == 3]`)

b. Antibiotic 1 and 4 (p = `r dlab_tukey$p.adj[dlab_tukey$term == "antibiotic" & dlab_tukey$group1 == 1 & dlab_tukey$group2 == 4]`)

c. Antibiotic 2 and 3 (p = `r dlab_tukey$p.adj[dlab_tukey$term == "antibiotic" & dlab_tukey$group1 == 2 & dlab_tukey$group2 == 3]`)

d. Antibiotic 2 and 4 (p = `r dlab_tukey$p.adj[dlab_tukey$term == "antibiotic" & dlab_tukey$group1 == 2 & dlab_tukey$group2 == 4]`)

e. Antibiotic 3 and 4 (p = `r dlab_tukey$p.adj[dlab_tukey$term == "antibiotic" & dlab_tukey$group1 == 3 & dlab_tukey$group2 == 4]`)

```{r message=F, warning=F, results=F}
mean_results <- dlabs %>%
  group_by(lab, antibiotic) %>%
  summarize(mean_results = mean(results)) %>% arrange(antibiotic)
mean_results


# Plot line graph of mean_results
ggplot(mean_results, aes(x = antibiotic, y = mean_results, color = lab, group = lab)) +
  geom_point() +
  geom_line() +
  labs(x = "Antibiotic", y = "Mean Results") +
  scale_color_discrete(name = "Lab") +
  theme_minimal()
```
